...
run_dir=${root}/dynamic_mooncake_abort_dev/monitoring/runs/mapre64
...
python3 -m mlflow server --host 127.0.0.1 --port "${port}" \
  --backend-store-uri "sqlite:///${run_dir}/mlflow.db" \
  --artifacts-destination "${run_dir}/artifacts" > "${run_dir}/mlflow-redump-server.log" 2>&1 &
pid=$!
trap 'kill "${pid}" 2>/dev/null || true; wait "${pid}" 2>/dev/null || true' EXIT
for _ in $(seq 1 120); do
  curl -fsS "${MLFLOW_TRACKING_URI}/health" >/dev/null && break
  sleep 1
done
curl -fsS "${MLFLOW_TRACKING_URI}/health"
python3 "${dump_script}" --output-dir "${run_dir}/trace_dump" \
  --project-name dynamic-recompute-moon-a-precheck-r64 \
  --expected-traces 128 --expected-server-replicas 8 --minimum-retry-attempts 0
