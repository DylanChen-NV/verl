...
export GPU_MEMORY_UTILIZATION=0.70 STANDALONE_GPU_MEMORY_UTILIZATION=0.70 ROLLOUT_ENFORCE_EAGER=False
...
WORKSPACE=${WORKSPACE:-${ROOT}/dynamic_mooncake_abort_dev}
REPO=${REPO:-${WORKSPACE}/verl_repo}
...
RUNTIME_PYTHONPATH="${PYDEPS_DIR}:${REPO}:${MLFLOW_SITE}${PYTHONPATH:+:${PYTHONPATH}}"
...
source "${WORKSPACE}/scripts/mooncake_abc_runtime.inc.sh"
...
# Each Slurm rank has its own container overlay, so patch the installed vLLM
# scheduler on every rank. The repository clone remains untouched by this step.
python3 -P - <<'PY_PATCH_RECOMPUTE_VLLM'
...
for module_import in ("json", "os", "socket"):
    import_line = f"import {module_import}\n"
    if import_line not in text:
        target = "import time\n"
        if target not in text:
            raise SystemExit("vLLM scheduler import target not found")
        text = text.replace(target, import_line + target, 1)
        changed = True

if "def _verl_recompute_request_meta(" not in text:
    class_index = text.find("\nclass Scheduler")
    if class_index < 0:
        raise SystemExit("vLLM Scheduler class target not found")
    helper = '''_VERL_RECOMPUTE_MARKER = "__verl_recompute_attempt_"


def _verl_recompute_request_meta(request_id: str):
    if _VERL_RECOMPUTE_MARKER not in request_id:
        return None
    logical_request_id, raw_attempt = request_id.rsplit(_VERL_RECOMPUTE_MARKER, 1)
    try:
        attempt_id = int(raw_attempt)
    except ValueError:
        return None
    return logical_request_id, attempt_id


def _verl_recompute_emit(phase: str, request_id: str, **fields) -> None:
    if os.getenv("VERL_RECOMPUTE_TRACE", "0") != "1":
        return
    meta = _verl_recompute_request_meta(request_id)
    if meta is None:
        return
    logical_request_id, attempt_id = meta
    payload = {
        "phase": phase,
        "logical_request_id": logical_request_id,
        "engine_request_id": request_id,
...
old = '''    def schedule(self) -> SchedulerOutput:
        self.current_step += 1
'''
new = '''    def schedule(self) -> SchedulerOutput:
        self.current_step += 1
        if not getattr(self, "_verl_enginecore_ready_emitted", False):
            self._verl_enginecore_ready_emitted = True
            if os.getenv("VERL_RECOMPUTE_TRACE", "0") == "1":
                print(
                    "VERL_RECOMPUTE_EVENT "
                    + json.dumps(
                        {
                            "phase": "ENGINECORE_READY",
                            "monotonic_ns": time.monotonic_ns(),
                            "wall_ns": time.time_ns(),
                            "node_id": socket.gethostname(),
                            "pid": os.getpid(),
...
if old in text:
    text = text.replace(old, new, 1)
    changed = True
elif '"phase": "ENGINECORE_READY"' not in text:
    raise SystemExit("vLLM schedule readiness target not found")

old = '''        for req_id, num_scheduled_token in num_scheduled_tokens.items():
            request = self.requests[req_id]
            request.num_computed_tokens += num_scheduled_token
'''
new = '''        for req_id, num_scheduled_token in num_scheduled_tokens.items():
            request = self.requests[req_id]
            external_request_id = req_id
            if _VERL_RECOMPUTE_MARKER in req_id and "-" in req_id:
                external_request_id, internal_suffix = req_id.rsplit("-", 1)
                if len(internal_suffix) != 8:
                    external_request_id = req_id
            request_meta = _verl_recompute_request_meta(external_request_id)
            scheduled_once = getattr(self, "_verl_recompute_scheduled_once", None)
            if scheduled_once is None:
                scheduled_once = set()
                self._verl_recompute_scheduled_once = scheduled_once
            if request_meta is not None and req_id not in scheduled_once:
                scheduled_once.add(req_id)
                _verl_recompute_emit(
                    "SCHEDULED",
                    external_request_id,
                    measurement_source="enginecore_scheduler",
                    internal_request_id=req_id,
                    retry_prefix_tokens=request.num_prompt_tokens,
                    initial_computed_tokens=request.num_computed_tokens,
                    num_scheduled_tokens=num_scheduled_token,
                    is_prefill=request.num_computed_tokens < request.num_prompt_tokens,
                    enginecore_pid=os.getpid(),
                    scheduler_file=__file__,
                )
            request.num_computed_tokens += num_scheduled_token
'''
if old in text:
    text = text.replace(old, new, 1)
    changed = True
elif "_verl_recompute_active" not in text:
    raise SystemExit("vLLM _update_after_schedule target not found")

old = '''            request = self.requests.get(req_id)
...
'''
new = '''            request = self.requests.get(req_id)
            if request is None or request.is_finished():
                # The request is already finished. This can happen if the
                # request is aborted while the model is executing it (e.g.,
                # in pipeline parallelism or in async scheduling).
            ...
            active = getattr(self, "_verl_recompute_active", {})
            state = active.get(req_id)
            if state is not None and request.num_computed_tokens >= request.num_prompt_tokens:
                _verl_recompute_emit(
                    "END",
                    req_id,
                    retry_prefix_tokens=request.num_prompt_tokens,
                    initial_computed_tokens=state["initial_computed_tokens"],
                    scheduled_prefill_tokens=state["scheduled_prefill_tokens"],
                )
...
if old in text:
    ...
    raise SystemExit("vLLM update_from_output target not found")

old = '''            valid_requests.append(request)
            if request.status == RequestStatus.RUNNING:
'''
new = '''            valid_requests.append(request)
            active = getattr(self, "_verl_recompute_active", {})
            state = active.pop(req_id, None)
...
if old in text:
    ...
    raise SystemExit("vLLM finish_requests target not found")

if changed:
    path.write_text(text)
    print(f"Applied vLLM recompute instrumentation to {path}")
else:
    print(f"vLLM recompute instrumentation already applied to {path}")
PY_PATCH_RECOMPUTE_VLLM
python3 "${WORKSPACE}/scripts/apply_vllm_mooncake_abort_reuse.py" --root /vllm
python3 "${WORKSPACE}/scripts/apply_mooncake_strict_worker_trace.py" --root /vllm
python3 "${WORKSPACE}/scripts/apply_mooncake_tcp_cpu_staging.py" --root /vllm
